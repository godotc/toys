...
Shader ResourceManager::LoadShader(const char *UniversalFileName, const char *name)
{
    ...
    if (!vertSource.empty() && !fragSource.empty()) {
        Shader shader;
        shader.Compile(vertSource.c_str(), fragSource.c_str(), gemoSource.empty() ? nullptr : gemoSource.c_str());
        Shaders[name] = shader;
        return shader;
    }
    ...
    if (vertSource.empty())
        LOG_PURE_ERROR("vertex are empty!");
    if (fragSource.empty())
        LOG_PURE_ERROR("frag are empty!");
...
{
    LOG_DEBUG("The file shader path is : {}", UniversalFilePath.c_str());
    if (UniversalFilePath.empty()) {
        LOG_PURE_ERROR("The file path is empty: {}", UniversalFilePath.c_str());
    }
    ...
    std::ifstream stream(UniversalFilePath.c_str());
    if (!stream.is_open() || stream.fail() || stream.bad()) {
        LOG_PURE_ERROR("Open this shader file failed!");
    }
